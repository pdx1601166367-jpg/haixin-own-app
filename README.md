...
- 双击即用，仅本机服务，无需登录或联网
- 数据保存在当前电脑的独立文件夹 `data/app.json` 中，刷新、关闭、重启后不丢失
...
Windows 下双击 `启动.bat`，macOS 下双击 `启动.command`。脚本会启动本机服务并自动打开浏览器访问 `http://127.0.0.1:4567`。

macOS 如果首次双击被系统拦截，在“系统设置 → 隐私与安全性”中允许运行，或在终端执行：
...
浏览器建议使用 Chrome 或 Edge。直接双击 `index.html` 仍可作为兼容模式打开，但此时数据会退回浏览器本地存储，不推荐日常使用。

## 数据与备份

- 数据保存在项目目录的独立文件 `data/app.json` 中，刷新、关闭浏览器、重启电脑后都不会丢失
- 复制或移动整个 `data` 文件夹即可完成手动备份
- 建议定期在“数据与设置 → 数据备份”中导出 JSON 备份
- 恢复导入会覆盖当前全部数据，导入前会要求二次确认
- 切换电脑或迁移时，先导出备份，或直接复制 `data/app.json`
